...
win.minsize(width=1325 , height=600)    #  最小尺寸
win.maxsize(width=1500, height=800)      #  最大尺寸
dateRes=""
newProduct=()
chartX=[]
chartY=[]
def addData():#新增
    ...
    for n in tree.get_children():
        ...
        plusID = idmax + 1
    newProduct=product(plusID,labelDateRes["text"],entryNameString.get(),entryYearString.get(),entryWeightString.get(),entryExhaustString.get(),
...
def changeData():#修改
    ...
    global temp
    changeProduct = product(entryIDString.get(),labelDateRes["text"],entryNameString.get(),entryYearString.get(),entryWeightString.get(),
                            entryExhaustString.get(),entryEngineString.get(),entryVariableString.get(),comboboxValueColor.get(),
                            entryAmountString.get(),comboboxValueStore.get(), entrySaleseString.get(),entryPriceString.get(), radioValue2.get())
...
color['values'] = ('競技藍', '深黑色', '競技白', '紅色','墨綠色')
...
productionUnit['values'] = ('桃園中正門市', '桃園中壢門市', '桃園青埔門市','桃園經國門市')
...
tree.place(x=10,y=350,width=1300)
